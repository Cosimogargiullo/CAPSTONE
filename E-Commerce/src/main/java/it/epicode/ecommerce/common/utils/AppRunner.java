package it.epicode.ecommerce.common.utils;
...
import it.epicode.ecommerce.models.ProductCatego;
import it.epicode.ecommerce.models.Role;
import it.epicode.ecommerce.models.RoleType;
...
import it.epicode.ecommerce.services.RoleService;
import it.epicode.ecommerce.services.ShopService;
...
//	CREATE USER
	private User initUser(List<Role> roles) {
		User user = new User();
		user.setEmail("cgomegaone@hotmail.com");
		user.setName("Cosimo");
		user.setSurname("Gargiullo");
		user.setUsername("c.gargiullo");
		user.setPassword(pe.encode("Test.123"));
...
//		user.setShop(new Shop());
	...
	private User initUser2(List<Role> roles) {
		User user = new User();
		user.setEmail("roselaura98@icloud.com");
		user.setName("Laura");
		user.setSurname("Gaudenci");
		user.setUsername("l.gaudencio");
		user.setPassword(pe.encode("Test.123"));
		roles.remove(0);
		user.setRoles(new HashSet<Role>(roles));
		user.getFavourite().setUser(user);
...
//	POPULATE DB FROM JSON
	public void populateProduct() throws EcommerceExceptionNotFound {
		...
		Shop shop = shopService.findById(1L).get();
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			String category = explrObject.getString("category").toUpperCase();
			category = category.replaceAll("\\s", "");
			String img = explrObject.getString("image");
			double rate = explrObject.getJSONObject("rating").getDouble("rate");
			int rateCount = explrObject.getJSONObject("rating").getInt("count");
